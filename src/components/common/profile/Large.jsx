...
export default function Large({ data, bgType }) {
  const { pt } = data;
  const borderData = data?.border || {};
  const avatar = data?.mobileAvatar ? data?.mobileAvatar : data.avatar;
    ...
    <div
      className={`relative w-full ${
        data?.mobileAvatar
          ...
          ? "max-w-[350px]"
          : "max-w-[580px]"
      }  mx-auto flex md:hidden w-full flex-col justify-center items-center pt-[2px]`}
    ...
    >
      ...
      <div className="avatar-bg flex absolute flex-col justify-center items-center pb-5 left-0 right-0 bottom-0">
        <div className="inline-flex relative mt-[15px] items-center">
          <p
            className="text-center text-xl md:text-[32px] 2xl:text-[40px] font-bold mr-[6px]"
          ...
          <svg
            ...
            className="absolute -right-[30px] sm:top-[7px]"
          >
            <path
